...
- Whole foods use a single raw/base entry; do not keep separate cooked or prepared whole-food variants. Meats usually use raw forms. Packaged, processed, or branded foods may be ranked in the prepared state they are sold or eaten in.
...
- Final production entries must fill every required visible submetric. Use the exact USDA/OFF record first. When it lacks a required value, search Google for that exact food identity, preparation state, metric, and per-100g basis; use the Google AI Overview as a discovery and estimate layer, checking its linked source or corroborating the value with another credible result before locking it in. Use another food or food-class analogue only as the last resort. Record whether the value is exact-source, Google-discovered, AI-Overview-estimated, or analogue-derived in provenance, but narrate the selected value directly and confidently. Never use an arbitrary invented number.
...
- Macro and micronutrient narration should try to mention the strongest and weakest defensible marks in each section, so sections usually have at least 2 nutrient/submacro points when 2 source-backed or approved display-backed values exist; explain what those marks are good or bad for, and for macros also explain why that macro section matters to the specific food type. For arrow submacros, stronger arrow bands outrank raw weighted score in narration selection. Do not use collagen as the weakest protein callout outside meats; prefer bioavailability or another relevant protein-quality mark. Low vitamin sections may be grouped as all-round low while still saying what vitamins matter for that food type.
