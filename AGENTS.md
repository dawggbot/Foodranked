...
- On-screen food names always display number words as numerals, for example `Zero-Sugar` becomes `0-Sugar` and `Two` becomes `2`; for header fit, prefer full or very lightly abbreviated names with dynamic font-size shrinking before using heavier abbreviations; shrunk header food names should anchor bottom-left so they still sit on the food-name line sprite.
