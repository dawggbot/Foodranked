...
- macro and micronutrient narration should try to mention the strongest and weakest/lowest defensible marks in each section when two source-backed or approved display-backed values exist; macro narration should also explain what selected outstanding submacros are good or bad for; every macro, vitamin, and mineral section should end with a quick food-type summary explaining why the mentioned scores matter to that category; arrow-band strength outranks raw weighted score for narration selection, collagen should not be used as the weak protein callout outside meats, and all-low vitamin sections may be grouped while still naming the category-specific vitamin role
...
- finalized foods fill every visible protein row using exact-food database data first, then exact-identity Google research with AI Overview values checked against a linked or corroborating source, and another protein analogue only as the last resort; provenance retains the source tier while narration states selected values confidently, and display-only defaults remain draft/runtime safeguards only
