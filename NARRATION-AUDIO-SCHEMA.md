...
Food-entry SFX variation is stored in `episode.sfxProfile`. Video Builder v2 should use `stampImpact.path` and `sectionTransition.path` from that profile when present, while keeping the default SFX constants as fallbacks. `sectionTransition.volume` may lift quieter transition files; values above `1` use a gain boost. Each referenced browser-facing `audio/sfx/...` path must have a matching `docs/audio/sfx/...` mirror.
