...
- display values for `omega3_mg` and `cholesterol_mg` convert to grams when the raw milligram number reaches 5 digits, for example `17830mg` displays as `17.83g`; the source metric and scoring value stay in milligrams
...
- food-type-specific weak-callout exceptions must stay narrow and explicit, not blanket-applied across every food type; current exceptions: low `starch_g` is not selected as the spoken carb weakness for fruits or dairy, low `vitamin_b12_dv` is not selected as the weak vitamin callout for plant food types, low protein-quality rows are not selected for fruits/vegetables/tubers below their useful-protein threshold, low omega-3/polyunsaturated-fat rows are not selected for fruits/vegetables/tubers/grains with `<=1g` total fat, and `0mg` cholesterol is not selected as a spoken fat standout outside meats/dairy; visible rows/arrows stay stable
