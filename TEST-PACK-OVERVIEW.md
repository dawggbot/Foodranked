...
After the 2026-04-02 audit, the intended interpretation is:
- rulesets should carry explicit category logic rather than generic fallback behaviour
- context-item scoring should match the ruleset policy exactly
- this pack is still a test harness, not a production nutrition database

Before production use:
- replace values with sourced canonical data
- expand tailored rulesets across the remaining food types
- pressure-test tier outcomes across many peer foods inside each category
- audit every context item for food-specific, non-redundant value
