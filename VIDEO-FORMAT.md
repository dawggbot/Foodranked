...
Food entries may override reusable SFX through `episode.sfxProfile`. Video Builder v2 should use `stampImpact.path` and `sectionTransition.path` from that profile when present, then fall back to the default constants. `sectionTransition.volume` may lift quieter transition files; values above `1` use a gain boost. The paths are browser-facing `audio/sfx/...` paths and must have matching mirrors under `docs/audio/sfx/...`.
...
- skip `N/A` or weakly sourced values, and do not use protein headline grams as the protein submacro or pad missing rows into narration just to hit two items; outside meats, do not use collagen as the weak protein callout when protein digestibility or another protein-quality mark is available
